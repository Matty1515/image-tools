...
useHead({
  title: 'Combine two pictures into one — Merge Tool'
})
...
<template>
  <div class="page">
    ...
    <div class="wrap">
      <hr class="rule" />
      <footer class="note">Images are combined locally in your browser. Nothing is uploaded to a server.</footer>
    </div>
  </div>
</template>
...
.wrap { max-width: 880px; margin: 0 auto; padding: 0 clamp(20px, 5vw, 56px); }
...
.hero { padding: clamp(40px, 8vw, 88px) 0 clamp(28px, 5vw, 48px); }
.display { font-family: var(--font-heading); font-weight: var(--font-heading-weight); font-size: clamp(32px, 5vw, 56px); line-height: 1.08; letter-spacing: -0.015em; margin: 0; max-width: 16ch; }
.hero .sub { font-size: 17px; line-height: 1.6; max-width: 58ch; color: color-mix(in srgb, var(--color-text) 80%, transparent); margin: var(--space-3) 0 0; }
.tool-section { padding: 0 0 clamp(40px, 6vw, 72px); }
.tool-card { display: grid; gap: var(--space-6); padding: var(--space-6); }
...
.dropzone {
  ...
  border: 1px dashed var(--color-neutral-600); background: var(--color-neutral-800);
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  gap: var(--space-2); cursor: pointer; overflow: hidden; text-align: center;
  transition: border-color 0.15s ease, background 0.15s ease;
}
.dropzone:hover { border-color: var(--color-neutral-500); }
.dropzone.drag-over { border-color: var(--color-accent); background: color-mix(in srgb, var(--color-accent-800) 40%, var(--color-neutral-800)); }
.dropzone .placeholder-icon { color: var(--color-neutral-500); }
.dropzone .placeholder-text { font-size: 13px; color: color-mix(in srgb, var(--color-text) 60%, transparent); padding: 0 var(--space-3); }
.dropzone .thumb { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: contain; background: var(--color-neutral-900); }
.dropzone .remove-btn {
  position: absolute; top: var(--space-2); right: var(--space-2); width: 28px; height: 28px;
...
.dropzone .remove-btn:hover { border-color: var(--color-accent); color: var(--color-accent); }
...
.preview-wrap canvas { max-width: 100%; max-height: 420px; height: auto; display: block; border-radius: var(--radius-sm); }
.preview-empty { font-size: 13px; color: color-mix(in srgb, var(--color-text) 55%, transparent); text-align: center; }
.actions { display: flex; justify-content: flex-end; align-items: center; gap: var(--space-3); }
.actions .hint { font-size: 12px; color: color-mix(in srgb, var(--color-text) 55%, transparent); margin-right: auto; }
...
.seg-opt:first-child { border-radius: var(--radius-md) 0 0 var(--radius-md); }
.seg-opt:last-child { border-radius: 0 var(--radius-md) var(--radius-md) 0; }
.nav-brand { display: inline-flex; align-items: center; gap: var(--space-2); }
